...
export const NodeInspector = ({ node, onControlNode }: NodeInspectorProps) => {
  ...
  if (!node) {
    return (
      <div className="glass-card p-6 flex items-center justify-center text-muted-foreground min-h-[200px]">
        <p>Click on a node in the topology map to inspect it</p>
      </div>
    );
  }
  ...
  return (
    <div className="glass-card p-6">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-border/50">
        <div className="flex items-center gap-6">
          ...
          <div className={cn(
            ...
            node.status === 'Active' && "bg-accent/20 text-accent",
            node.status === 'Down' && "bg-destructive/20 text-destructive",
            node.status === 'Syncing' && "bg-warning/20 text-warning"
          )}>
            {node.status}
          </div>
        </div>
      ...
      {/* Grid Layout for Inspector Content */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Metrics Gauges */}
        <div className="lg:col-span-1">
          <h4 className="text-sm font-medium text-foreground mb-4">System Metrics</h4>
          <div className="flex justify-around lg:flex-col lg:items-center lg:gap-4">
            <GaugeCircle
        ...
        <div className="lg:col-span-1">
          ...
          <pre className="p-3 rounded-lg bg-secondary/50 border border-border/50 text-xs font-mono text-muted-foreground overflow-x-auto max-h-40 overflow-y-auto">
            {node.config}
          </pre>
        </div>
      </div>
